...
EXPOSE 7000
CMD ["sh", "-c", "node scripts/migrate-shopping-item-unit-key.mjs && npx prisma db push && node scripts/migrate-attendance.mjs && node scripts/ensure-catalog.mjs && exec node scripts/start.mjs"]
