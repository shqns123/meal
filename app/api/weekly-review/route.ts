...
function validDate(value?: string) { return /^\d{4}-\d{2}-\d{2}$/.test(value ?? "") && !Number.isNaN(new Date(`${value}T00:00:00Z`).valueOf()); }
function sundayFor(date: string) { return addDays(date, -new Date(`${date}T00:00:00Z`).getUTCDay()); }
...
export async function GET(request: Request) {
  ...
  if (!validDate(referenceDate)) return NextResponse.json({ error: "date must be in YYYY-MM-DD format" }, { status: 400 });
  ...
  return NextResponse.json({ review: review ? { ...review, referenceDate: review.referenceDate ? formatKst(review.referenceDate) : weekStart } : null, pantry: pantry.map((item) => ({ ...item, expiresAt: item.expiresAt ? formatKst(item.expiresAt) : null })), schedules: schedules.map((item) => ({ ...item, date: formatKst(item.date) })) });
}
