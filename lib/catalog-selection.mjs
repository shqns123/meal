...
export function chooseCatalogMenu({ catalog, history = [], sourceCategories, date, seed }) {
  ...
  const ageFor = (predicate, role) => {
    const ages = history
      .filter((entry) => (!role || !entry.selectionRole || entry.selectionRole === role) && predicate(entry))
      .map((entry) => daysAgo(today, entry.date))
      .filter((age) => age >= 0);
    return ages.length ? Math.min(...ages) : Infinity;
  };
  const score = (relaxVariantCooldown) => {
    const scored = [...grouped.values()].map((base) => {
      const role = base.selectionRole ?? catalogSelectionRole(base.sourceCategory);
      const usable = base.variants.flatMap((variant) => {
        const variantAge = ageFor((entry) => entry.variantName === variant.variantName, null);
        if (!relaxVariantCooldown && variantAge < COOLDOWNS.variantDays) return [];
        const similarAge = variant.similarGroup
          ? ageFor((entry) => entry.similarGroup === variant.similarGroup, role) : Infinity;
        const ingredientAge = variant.primaryIngredient
          ? ageFor((entry) => entry.primaryIngredient === variant.primaryIngredient, role) : Infinity;
        const flavorAge = variant.flavorFamily && variant.flavorFamily !== "담백"
          ? ageFor((entry) => entry.flavorFamily === variant.flavorFamily, role) : Infinity;
      ...
      if (!usable.length) return { ...base, usable, baseWeight: 0 };
      const baseAge = ageFor((entry) => entry.baseName === base.baseName, role);
      let baseWeight = baseAge < COOLDOWNS.baseStrongDays ? 0.05
        : baseAge < COOLDOWNS.baseSoftDays ? 0.25 : 1;
      // 평균을 쓰므로 세부메뉴 개수가 늘어도 기본메뉴의 기본 확률은 늘지 않는다.
      baseWeight *= usable.reduce((sum, variant) => sum + variant.weight, 0) / usable.length;
      const familyAge = ageFor((entry) => entry.cookingFamily === base.baseCookingFamily, role);
      const recentFamilyDates = new Set(history
        .filter((entry) => (!entry.selectionRole || entry.selectionRole === role)
          && entry.cookingFamily === base.baseCookingFamily
          && daysAgo(today, entry.date) >= 1 && daysAgo(today, entry.date) <= 2)
        .map((entry) => entry.date));
      const cookingWeight = recentFamilyDates.size === 2 ? 0.12
        : familyAge < COOLDOWNS.cookingFamilyDays ? 0.45 : 1;
      return { ...base, usable, baseWeight, cookingWeight };
    });
    const familyGroups = new Map();
    for (const base of scored) {
      if (!base.usable.length) continue;
      const family = familyGroups.get(base.baseCookingFamily)
        ?? { cookingFamily: base.baseCookingFamily, bases: [], weight: base.cookingWeight };
      family.bases.push(base);
      familyGroups.set(base.baseCookingFamily, family);
    }
    for (const family of familyGroups.values()) {
      // 한 조리계열 안의 후보가 전부 최근 먹은 유사메뉴라면 계열 자체도 덜 뽑는다.
      // 평균을 사용해 기본메뉴·세부메뉴 개수로 계열 확률을 늘리지 않는다.
      family.weight *= family.bases.reduce((sum, base) => sum + base.baseWeight, 0) / family.bases.length;
    }
    return [...familyGroups.values()];
  };
  // 허용 후보가 모두 최근에 사용된 경우에만 동일 세부메뉴를 낮은 확률로 재허용한다.
  let families = score(false);
  if (!families.length) families = score(true);
  const selectedFamily = weightedPick(families, random);
  const selectedBase = selectedFamily && weightedPick(
    selectedFamily.bases.map((base) => ({ ...base, weight: base.baseWeight })), random);
