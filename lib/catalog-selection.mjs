...
export function chooseCatalogMenu({ catalog, history = [], sourceCategories, date, seed }) {
  ...
  const score = ({ relaxVariant = false, relaxBase = false, relaxSimilar = false, relaxSideFamily = false }) => {
    const scored = [...grouped.values()].flatMap((base) => {
      ...
      return [{ ...base, usable, configuredWeight, baseWeight: usable.reduce((sum, variant) => sum + variant.weight, 0) / usable.length * configuredWeight, cookingWeight }];
    });
    const highestPriority = scored.reduce((highest, base) => Math.max(
      highest,
      ...base.usable.map((variant) => Number(variant.selectionPriority) || 0),
    ), 0);
    const prioritized = highestPriority > 0
      ? scored.flatMap((base) => {
          const usable = base.usable.filter(
            (variant) => (Number(variant.selectionPriority) || 0) === highestPriority,
          );
          if (!usable.length) return [];
          return [{
            ...base,
            usable,
            baseWeight: usable.reduce((sum, variant) => sum + variant.weight, 0)
              / usable.length * base.configuredWeight,
          }];
        })
      : scored;
