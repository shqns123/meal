...
- OpenRouter 직접 연동 식단 작업·웹 검색 레시피 검증·읽기 전용 채팅
...
`.env`에 `OPENROUTER_API_KEY`와 `OPENROUTER_MODEL`을 설정하면 식단 생성·수정, 레시피 재생성, 장보기 재생성, 주간 점검, 읽기 전용 채팅을 앱에서 직접 처리합니다. 레시피와 채팅에는 기본적으로 OpenRouter 웹 검색·본문 읽기를 함께 요청합니다.
